...
const flattenAttributes = (attributes, result = []) => {
    attributes.forEach((attribute) => {
        result.push(attribute);
        if (attribute.attribute_values && attribute.attribute_values.length > 0) {
            result.push(...attribute.attribute_values);
        }
    });
    return result;
};
...
const filteredAttributes = computed(() => {
    ...
    const flattenedList = flattenAttributes(allAttributes.value);
    const filtered = flattenedList.filter(
        (item) =>
            item.name?.toLowerCase().includes(lowerCaseSearch) ||
            item.value_name?.toLowerCase().includes(lowerCaseSearch) ||
            item.type?.toLowerCase().includes(lowerCaseSearch)
    );

    const uniqueAttributeIds = [
        ...new Set(filtered.map((item) => item.attribute_id || item.id)),
    ];
    return allAttributes.value.filter((attribute) =>
        uniqueAttributeIds.includes(attribute.id)
    );
});
