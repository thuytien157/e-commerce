            ...
            <div class="product-sidebar">
              ...
              <div v-for="attribute in attributes" :key="attribute.id">
                <div class="single-widget condition">
                  ...
                  <div v-else :class="{
                    'd-flex flex-column gap-2': attribute.name !== 'Màu sắc',
                    'd-flex flex-column gap-2': attribute.name === 'Màu sắc',
                  }">
                    <div v-for="value in attribute.attribute_values" :key="value.id">
                      <template v-if="attribute.name === 'Màu sắc'">
                        <div class="d-inline-block rounded-circle" :class="{
                          'border-2 border-primary': isAttributeSelected(attribute.id, value.value_name),
                          'color-selected': isAttributeSelected(attribute.id, value.value_name)
                        }" @click="toggleAttributeFilter(attribute.id, value.value_name)" :style="{
                          width: '1.35rem',
                          height: '1.35rem',
                          'background-color': value.value_name,
                          cursor: 'pointer',
                          border: isAttributeSelected(attribute.id, value.value_name) ? '2px solid #0d6efd' : '1px solid #ccc'
                        }"></div>
                      </template>
                      <template v-else>
                        <div class="form-check">
